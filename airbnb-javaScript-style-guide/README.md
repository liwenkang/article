...
本文是为了记录在使用 JavaScript 中代码风格方面需要注意的问题,参考 [airbnb](https://github.com/airbnb/javascript) 的代码规范
...
- [1.1](#types--primitives) **Primitives**: When you access a primitive type you work directly on its value.
  ...
  - 业务中还没用过 symbol 类型,它可以用来表示独一无二的值(针对类型判断,需要再总结一下)
...
- [1.2](#types--complex)  **Complex**: When you access a complex type you work on a reference to its value.
  ...
  对于复杂的数据类型,直接赋值,拿到的是引用,所以衍生出了浅复制/深复制的问题
...
- [2.1](#references--prefer-const) Use `const` for all of your references; avoid using `var`. eslint: [`prefer-const`](https://eslint.org/docs/rules/prefer-const.html), [`no-const-assign`](https://eslint.org/docs/rules/no-const-assign.html)
  ...
  对于赋值操作,除非之后要改变的,统统用 const 替代 var
...
- [2.2](#references--disallow-var) If you must reassign references, use `let` instead of `var`. eslint: [`no-var`](https://eslint.org/docs/rules/no-var.html)
  ...
  如果需要对参数重新进行赋值,就用 let (具有块级作用域,引出了 var,let,const 的区别)
...
- [2.3](#references--block-scope) Note that both `let` and `const` are block-scoped.
  ...
  相对来说,使用 let, const 这种具有块级作用域的变量,将会避免全局变量污染问题
...
- [3.2](#es6-computed-properties) Use computed property names when creating objects with dynamic property names.
  ...
  尽量避免在对象外给这个对象添加新的属性, 尽可能在对象初始化的时候,就完成定义
...
- [3.3](#es6-object-shorthand) Use object method shorthand. eslint: [`object-shorthand`](https://eslint.org/docs/rules/object-shorthand.html)
  ...
  如果一个对象中的某个属性是一个函数的话,应该使用省略function 的简写形式

  比如在vue中,子组件中的 data  是一个函数,就应该写成 data () {}, 对于一些生命周期函数,比如 created, 也应该写成 created () {}
...
- [3.4](#es6-object-concise) Use property value shorthand. eslint: [`object-shorthand`](https://eslint.org/docs/rules/object-shorthand.html)
  ...
  如果属性名和属性值相同的话,应该使用简写形式(此处可以涉及到 es6 中的解构赋值,需要重点看下对象的解构赋值问题)
...
- [3.5](#objects--grouped-shorthand) Group your shorthand properties at the beginning of your object declaration.
  ...
  针对属性名和属性值相同的情况下,缩写的部分应该放在对象的前面
...
- [3.7](#objects--prototype-builtins) Do not call `Object.prototype` methods directly, such as `hasOwnProperty`, `propertyIsEnumerable`, and `isPrototypeOf`. eslint: [`no-prototype-builtins`](https://eslint.org/docs/rules/no-prototype-builtins)
  ...
  为了避免一些边界情况, 在使用 prototype 上的方法的时候, 建议先在模块内做一次缓存, 然后再调用,或者直接使用成熟的库解决.
...
- [4.8](#arrays--bracket-newline) Use line breaks after open and before close array brackets if an array has multiple lines
  ...
  要是横着,就全部横着,要么就拆成一行一行的....(这个要多注意....我之前写的就非常符合 bad 的要求)
...
- [5.1](#destructuring--object) Use object destructuring when accessing and using multiple properties of an object. eslint: [`prefer-destructuring`](https://eslint.org/docs/rules/prefer-destructuring)
  ...
  // best
  // 传入的依旧是对象,但是在形参阶段就通过解构赋值...从而在后面方便的使用
...
- [5.3](#destructuring--object-over-array) Use object destructuring for multiple return values, not array destructuring.
  ...
  如果需要返回多个值,那么用对象解构,而不是用数组(之前没注意过这点啊.....)
...
- [7.1](#functions--declarations) Use named function expressions instead of function declarations. eslint: [`func-style`](https://eslint.org/docs/rules/func-style)
  ...
  // good
  // 表达式的名字(长的,详细的函数描述) 和 变量引用的调用(短的,方便使用)
...
- [7.3](#functions--in-blocks) Never declare a function in a non-function block (`if`, `while`, etc). Assign the function to a variable instead. Browsers will allow you to do it, but they all interpret it differently, which is bad news bears. eslint: [`no-loop-func`](https://eslint.org/docs/rules/no-loop-func.html)
  ...
  不要在 if 或是 loop 中定义函数,应该把函数分配给一个变量
...
- [7.4](#functions--note-on-blocks) **Note:** ECMA-262 defines a `block` as a list of statements. A function declaration is not a statement.
  ...
  标准对于 块 的定义是一系列语句,函数声明不是语句
  在特殊条件下,也应该先声明变量,再定义函数
...
- [7.5](#functions--arguments-shadow) Never name a parameter `arguments`. This will take precedence over the `arguments` object that is given to every function scope.
  ...
  别把 arguments 作为形参, 因为它在函数中是一个特殊的数组
...
- [7.6](#es6-rest) Never use `arguments`, opt to use rest syntax `...` instead. eslint: [`prefer-rest-params`](https://eslint.org/docs/rules/prefer-rest-params)
  ...
  用 ...args 代替 arguments,因为这样拿到的是一个真正的数组,而不是一个类数组.对于处理剩余参数也更友好
...
- [7.10](#functions--constructor) Never use the Function constructor to create a new function. eslint: [`no-new-func`](https://eslint.org/docs/rules/no-new-func)
  ...
  别用 Function 构造函数来创建一个函数.因为用到了字符串解析(将会和用 eval 一样出问题)
...
- [7.11](#functions--signature-spacing) Spacing in a function signature. eslint: [`space-before-function-paren`](https://eslint.org/docs/rules/space-before-function-paren) [`space-before-blocks`](https://eslint.org/docs/rules/space-before-blocks)
  ...
  const x = function () {};
  const y = function a() {};
  const shortFunctionName = function detailFunctionName() {}
  ```
...
- [7.12](#functions--mutate-params) Never mutate parameters. eslint: [`no-param-reassign`](https://eslint.org/docs/rules/no-param-reassign.html)
  ...
  操作作为参数传入的对象将会对原始调用者产生意想不到的副作用.所以,应该把参数存起来一份,然后再进行之后的操作.
...
- [7.13](#functions--reassign-params) Never reassign parameters. eslint: [`no-param-reassign`](https://eslint.org/docs/rules/no-param-reassign.html)
  ...
  如果要对参数重新赋值的话...应该用默认参数,或者在函数里面先把参数传递给一个变量保存(总之就是不要对参数进行直接修改)
...
- [7.14](#functions--spread-vs-apply) Prefer the use of the spread operator `...` to call variadic functions. eslint: [`prefer-spread`](https://eslint.org/docs/rules/prefer-spread)
  ...
  使用 ... 运算符替代 new 和 apply ,用在时间格式化上很好用...
...
- [7.15](#functions--signature-invocation-indentation) Functions with multiline signatures, or invocations, should be indented just like every other multiline list in this guide: with each item on a line by itself, with a trailing comma on the last item. eslint: [`function-paren-newline`](https://eslint.org/docs/rules/function-paren-newline)
  ...
  有多个参数时,要么就是一行,要么就散开(注意结尾处的 , 要写上,保证统一)
...
- [8.2](#arrows--implicit-return) If the function body consists of a single statement returning an [expression](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Expressions_and_Operators#Expressions) without side effects, omit the braces and use the implicit return. Otherwise, keep the braces and use a `return` statement. eslint: [`arrow-parens`](https://eslint.org/docs/rules/arrow-parens.html), [`arrow-body-style`](https://eslint.org/docs/rules/arrow-body-style.html)
  ...
  这里不是很清楚, 如果说函数是一个纯函数(没有副作用),且函数体内就是一个表达式,那就省略 return 和 {}
...
- [8.4](#arrows--one-arg-parens) If your function takes a single argument and doesn’t use braces, omit the parentheses. Otherwise, always include parentheses around arguments for clarity and consistency. Note: it is also acceptable to always use parentheses, in which case use the [“always” option](https://eslint.org/docs/rules/arrow-parens#always) for eslint. eslint: [`arrow-parens`](https://eslint.org/docs/rules/arrow-parens.html)
  ...
  在使用箭头函数的时候,如果函数体没有大括号,而且参数只有一个,那就省略包裹形参的小括号
  如果函数体有大括号,即使形参只有一个,也建议加上包裹形参的小括号
...
- [9.1](#constructors--use-class) Always use `class`. Avoid manipulating `prototype` directly.
  ...
  使用 class 的语法, 而不是直接操作 prototype 
...
- [9.3](#constructors--chaining) Methods can return `this` to help with method chaining.
  ...
  为了实现链式调用(方法链), 在 class 中定义的方法建议返回 this
...
- [9.5](#constructors--no-useless) Classes have a default constructor if one is not specified. An empty constructor function or one that just delegates to a parent class is unnecessary. eslint: [`no-useless-constructor`](https://eslint.org/docs/rules/no-useless-constructor)
  ...
  类有默认的构造方法(不需要写一个空的构造函数),以及只代表父类的构造函数是不需要写的
  像 react 里面继承出来的子组件就一直在写
  ```js
...
- [10.1](#modules--use-them) Always use modules (`import`/`export`) over a non-standard module system. You can always transpile to your preferred module system.
  ...
  使用 import 和 export (配合对象的解构赋值),就可以拿到需要的模块
...
- [10.2](#modules--no-wildcard) Do not use wildcard imports.
  ...
  在使用 import 的时候,不要使用通配符,而是应该导出单一的功能
...
- [10.8](#modules--multiline-imports-over-newlines) Multiline imports should be indented just like multiline array and object literals.
  ...
  在 import 时,如果是多行内容,应该和对象/数组这种风格统一
...
在 import 语句中不要用 webpack 的语法(喵喵喵, webpack 还有专门的语法???)  
...
- [11.1](#iterators--nope) Don’t use iterators. Prefer JavaScript’s higher-order functions instead of loops like `for-in` or `for-of`. eslint: [`no-iterator`](https://eslint.org/docs/rules/no-iterator.html) [`no-restricted-syntax`](https://eslint.org/docs/rules/no-restricted-syntax)
  ...
  用高阶函数来代替 for 遍历(for循环, for-in, for-of ),使用纯函数(针对数组的: forEach, map, every, filter, find, findIndex, reduce, some; 针对对象的: Object.keys, Object.values, Object.entries)避免副作用.
  注意,在开发过程中,遇到了需要将一个包含了很多对象的数组进行遍历,选取对象的部分属性重新填充到一个数组中的情况,目前比较好用的方法就是 forEach / map ,但还是比较繁琐,可以改进
...
- [13.4](#variables--define-where-used) Assign variables where you need them, but place them in a reasonable place.
  ...
  变量的声明要在需要用到它的时候做,而不是全部放在函数起始位置
...
- [13.6](#variables--unary-increment-decrement) Avoid using unary increments and decrements (`++`, `--`). eslint [`no-plusplus`](https://eslint.org/docs/rules/no-plusplus)
  ...
  ++
  b
  // 解析为 a; ++b,为避免这种情况发生,应该永远使用 num += 1
  // 如果第二行首字元是 "("、"["、"/"、"+"、"-", 那么不会自动添加分号,
  ```
...
- [14.1](#hoisting--about) `var` declarations get hoisted to the top of their closest enclosing function scope, their assignment does not. `const` and `let` declarations are blessed with a new concept called [Temporal Dead Zones (TDZ)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/let#Temporal_Dead_Zone). It’s important to know why [typeof is no longer safe](http://es-discourse.com/t/why-typeof-is-no-longer-safe/15).
  ...
  let/const 没有声明提升
  typeof 运算符在对使用 let/const 声明的变量(未赋值)的时候,将会报错
...
- [14.2](#hoisting--anon-expressions) Anonymous function expressions hoist their variable name, but not the function assignment.
  ...
  匿名函数表达式的情况和使用 var 的情况想死,具有声明提升,但函数体不提升
...
- [15.2](#comparison--if) Conditional statements such as the `if` statement evaluate their expression using coercion with the `ToBoolean` abstract method and always follow these simple rules:
  ...
  if 语句遵守以下规则:
    对象就是 true
    undefined, null 就是 false
    数字里的 +0,-0,NaN 是 false
    空字符串是 false
...
- [15.3](#comparison--shortcuts) Use shortcuts for booleans, but explicit comparisons for strings and numbers.
  ...
  对于显而易见的布尔类型,if 中不需要多写一次比较,但是对于字符串和数字,要有具体的比较值
...
- [17.1](#control-statements) In case your control statement (`if`, `while` etc.) gets too long or exceeds the maximum line length, each (grouped) condition could be put into a new line. The logical operator should begin the line.
  ...
  如果有多个条件的话,一行写不下,就分散开写成多行,逻辑操作符写前面(&&)
...
- [21.1](#semicolons--required) **Yup.** eslint: [`semi`](https://eslint.org/docs/rules/semi.html)
  ...
  原本是一个不加分号党...
  如果要选择这条冒险的路,就要注意什么情况下会触发 bug(自动插入分号时出现的问题)
  1. ++ -- 这种,前面不插入分号
  ```js
  a(这里不插入分号)
  ++
  ...
  2. 立即执行函数,注意这个函数之前,最好加上分号,避免立即执行函数作为参数传入函数之类
  3. return 后面要紧跟返回的内容,如果换行了,之后的内容会丢失
...
- [22.3](#coercion--numbers) Numbers: Use `Number` for type casting and `parseInt` always with a radix for parsing strings. eslint: [`radix`](https://eslint.org/docs/rules/radix) [`no-new-wrappers`](https://eslint.org/docs/rules/no-new-wrappers)
  ...
  Number() 或者 parseInt(,参数) 是比较好的转换为数字的方法
...
- [22.5](#coercion--bitwise) **Note:** Be careful when using bitshift operations. Numbers are represented as [64-bit values](https://es5.github.io/#x4.3.19), but bitshift operations always return a 32-bit integer ([source](https://es5.github.io/#x11.7)). Bitshift can lead to unexpected behavior for integer values larger than 32 bits. [Discussion](https://github.com/airbnb/javascript/issues/109). Largest signed 32-bit Int is 2,147,483,647:
  ...
  数字是用64位表示的,但是移位运算符返回的是32位数字
  当移位运算的数字超过32位的时候,将会出现意外
  判断是否是偶数 return !!(n & 1); 这种操作就很秀
...
- [23.3](#naming--PascalCase) Use PascalCase only when naming constructors or classes. eslint: [`new-cap`](https://eslint.org/docs/rules/new-cap.html)
  ...
  首字母大写只用在 class 和 constructor 上
...
- [23.5](#naming--self-this) Don’t save references to `this`. Use arrow functions or [Function#bind](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/bind).
  ...
  不要手动保存对 this 的引用,必要的时候,使用箭头函数/bind,拿 this
...
- [23.6](#naming--filename-matches-export) A base filename should exactly match the name of its default export.
  ...
  从一个文件中导出的默认模块应该和这个文件的文件名相同, import 的参数应该也是相同的
...
- [23.7](#naming--camelCase-default-export) Use camelCase when you export-default a function. Your filename should be identical to your function’s name.
  ...
  导出一个函数的时候,用驼峰法命名.文件名业内广告和函数名相同.
...
- [23.8](#naming--PascalCase-singleton) Use PascalCase when you export a constructor / class / singleton / function library / bare object.
  ...
  导出一个构造函数/类/单例(涉及到设计模式...)/函数库/对象的时候,用 PascalCase 
...
- [23.10](#naming--uppercase) You may optionally uppercase a constant only if it 
(1) is exported, 
...
(3) the programmer can trust it (and its nested properties) to never change.
  ...
  对于一个静态变量,如果它是作为导出变量,由 const 定义,子属性都是不可更改的,可以写成全大写字母
...
- [29.1](#standard-library--isnan) Use `Number.isNaN` instead of global `isNaN`.
  ...
  全局的 isNaN 不仅仅会对 NaN 返回 true, 还会把所有非数字转换为数字,再将转换结果为 NaN 的返回 true,实际上,我们希望 isNaN 仅仅判断一个值是不是 NaN ,所以最好使用 Number.isNaN()
...
- [29.2](#standard-library--isfinite) Use `Number.isFinite` instead of global `isFinite`.
  ...
  同样, 全局的 isFinite 也会对非数字类型做强制类型转换,然后再把转换结果为 Finite 的返回 true
  而 Number.isFinite 不会做类型转换,而是直接判断,这个值是不是 Finite
  总之,要尽量避免隐式类型转换
